...
export class NetworkEdgeRenderStrategy {
	...
	static evaluate(edge: GraphEdge, context: NetworkEdgeEvaluationContext): NetworkEdgeRenderDescriptor {
		...
		if (edgeKind === 'dependency') {
			...
			const alpha = zoom < 0.6 ? 0.65 : (zoom < 1.2 ? 0.45 : 0.25);
			return {
				variant: 'dependency',
		...
		if (isDynamic) {
			...
			return {
				...
				lineWidth: 1.1 / zoom,
				alpha: visible ? (zoom >= 0.8 ? 0.7 : 0.45) : 0,
				dash: [4, 4],
				curvature: 0,
				hasArrow: zoom >= 0.85,
				priority: 2,
		...
		const alpha = zoom < 0.6 ? 0.18 : (zoom < 1.1 ? 0.32 : 0.48);

		return {
			variant: 'import',
			strokeStyle: isHighContrast ? 'rgba(255, 255, 255, 0.4)' : 'rgba(148, 163, 184, 0.3)',
			lineWidth: 0.85 / zoom,
			alpha: visibleAtLOD ? alpha : 0,
			dash: [],
			curvature: 0,
			hasArrow: zoom >= 1.1,
			priority: 1,
