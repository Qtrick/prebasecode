...
| **Core - Typecheck** | TypeScript 7 `tsc` `noEmit` watch (type errors only, no output) | `watch-clientd` → `gulp watch-client` (uses `watchTypeCheckTask`) |
| **Ext - Build** | Extension compilation (unchanged) | `watch-extensionsd` |

### Key Changes

- **`build/lib/compilation.ts`**: `ICompileTaskOptions` gained `noEmit?: boolean`. `watchTypeCheckTask()` runs the TypeScript 7 compiler (`build/lib/typescriptCompiler.ts`) in watch mode with `noEmit: true`.
