...
import { isMagnusDefaultChatAgent } from '../../chatSetup/chatSetup.js';
...
/**
 * Id of the synthetic "Sign in / configure..." entry shown when Chat still
 ...
 * setup header and action (invoking `onRequestSetup`) replace all of the above.
 * `restrictedMode` takes precedence.
 */
export function buildModelPickerItems(
	...
	if (setupRequired) {
		...
		// usable. Surface a setup action (mirroring the send-message setup
		...
		items.push({
			...
			kind: ActionListItemKind.Action,
			label: actionLabel,
			group: { title: '', icon: ThemeIcon.fromId(magnus ? Codicon.key.id : Codicon.signIn.id) },
