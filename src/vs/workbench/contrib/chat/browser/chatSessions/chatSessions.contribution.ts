...
/** Copilot-bundled session types hidden when PreBase Magnus is the default agent. */
const COPILOT_DEFAULT_SESSION_TYPES = new Set<string>([
	SessionType.CopilotCLI,
	SessionType.CopilotCloud,
...
function isMagnusDefaultChatAgent(): boolean {
	return product.defaultChatAgent?.extensionId === 'prebase.magnus'
		|| product.defaultChatAgent?.chatExtensionId === 'prebase.magnus';
}

function shouldSuppressCopilotSessionType(type: string): boolean {
	return isMagnusDefaultChatAgent() && (COPILOT_DEFAULT_SESSION_TYPES.has(type) || type.startsWith('agent-host-'));
}
...
export class ChatSessionsService extends Disposable implements IChatSessionsService {
	...
	private registerContribution(contribution: IChatSessionsExtensionPoint, ext: IRelaxedExtensionDescription): IDisposable {
		...
		if (shouldSuppressCopilotSessionType(contribution.type)) {
			this._logService.trace(`[ChatSessionsService] registerContribution: type='${contribution.type}' suppressed for PreBase Magnus default agent`);
