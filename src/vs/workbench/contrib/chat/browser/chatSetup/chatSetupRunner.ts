...
import { IChatSetupResult, ChatSetupAnonymous, InstallChatEvent, InstallChatClassification, ChatSetupStrategy, ChatSetupResultValue, isMagnusDefaultChatAgent } from './chatSetup.js';
...
export class ChatSetup {
	...
	/**
	 * Magnus (PreBase) setup path: never prompt for an API key in the Agents
	 * window. Keys come from the gitignored project `.env` (at least one
	 * provider). Mark chat setup completed so Copilot sign-in is skipped.
	 */
	private async doRunMagnusSetup(options: { disableChatViewReveal?: boolean } | undefined, dialogSkipped: boolean): Promise<IChatSetupResult> {
		const trusted = await this.workspaceTrustRequestService.requestWorkspaceTrust({
			message: localize('chatWorkspaceTrust', "AI features are currently only supported in trusted workspaces.")
		});
		if (!trusted) {
			this.context.update({ later: true });
			this.telemetryService.publicLog2<InstallChatEvent, InstallChatClassification>('commandCenter.chatInstall', { installResult: 'failedNotTrusted', installDuration: 0, signUpErrorCode: undefined, provider: 'magnus' });
		...
		// Optional: refresh Magnus key detection (no modal). Missing keys are
		// handled when the user sends a chat message.
		try {
			await this.commandService.executeCommand('prebase.magnus.refreshToken');
		} catch {
			// Extension may still be activating.
		}

		this.context.update({ completed: true });
		this.chatEntitlementService.markSetupCompleted();
		this.telemetryService.publicLog2<InstallChatEvent, InstallChatClassification>('commandCenter.chatInstall', { installResult: 'installed', installDuration: 0, signUpErrorCode: undefined, provider: 'magnus' });

		try {
			await this.commandService.executeCommand('prebase.magnus.open');
		} catch {
			// Extension may still be activating; chat view is already revealed.
		}
	...
	private createDialogFooter(disposables: DisposableStore, options?: { forceAnonymous?: ChatSetupAnonymous }): HTMLElement {
		...
		let footer: string;
		if (isMagnusDefaultChatAgent()) {
			footer = localize({ key: 'settingsMagnus', comment: ['{Locked="["}', '{Locked="]({1})"}', '{Locked="]({2})"}'] }, "By continuing, you agree to {0}'s [Terms]({1}) and [Privacy Statement]({2}).", defaultChat.provider.default.name, defaultChat.termsStatementUrl || '#', defaultChat.privacyStatementUrl || '#');
