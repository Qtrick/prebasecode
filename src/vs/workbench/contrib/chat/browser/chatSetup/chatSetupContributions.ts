...
import { buildUpgradeUrlWithRedirect, ChatSetupAnonymous, ChatSetupStrategy, isMagnusDefaultChatAgent, refreshTokens } from './chatSetup.js';
...
export const CHAT_MARK_SETUP_COMPLETED_ACTION_ID = 'workbench.action.chat.markSetupCompleted';
...
export class ChatSetupContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerActions(context: ChatEntitlementContext, requests: ChatEntitlementRequests, controller: Lazy<ChatSetupController>): void {
		...
		class ChatMarkSetupCompletedAction extends Action2 {
			constructor() {
				super({
					id: CHAT_MARK_SETUP_COMPLETED_ACTION_ID,
					title: localize2('markChatSetupCompleted', "Mark Chat Setup Completed"),
					f1: false,
					category: CHAT_CATEGORY,
				});
			}
			...
			override async run(accessor: ServicesAccessor): Promise<void> {
				...
				const configurationService = accessor.get(IConfigurationService);
				// Overloads are exclusive — call separately for hidden vs completed.
				await context.update({ hidden: false });
				...
				super({
					...
					menu: {
						...
						when: ContextKeyExpr.and(
							...
							ChatContextKeys.Setup.completed.negate(),
							isMagnusDefaultChatAgent() ? ContextKeyExpr.true() : ChatContextKeys.Entitlement.signedOut
						)
					}
				});
			}
		...
		registerAction2(ChatMarkSetupCompletedAction);
