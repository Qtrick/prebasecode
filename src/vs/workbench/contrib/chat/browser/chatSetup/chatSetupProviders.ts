...
import { ChatSetupAnonymous, ChatSetupStep, IChatSetupResult, isMagnusDefaultChatAgent, maybeEnableAuthExtension, refreshTokens } from './chatSetup.js';
...
export class SetupAgent extends Disposable implements IChatAgentImplementation {
	...
	static registerDefaultAgents(instantiationService: IInstantiationService, location: ChatAgentLocation, mode: ChatModeKind, context: ChatEntitlementContext, controller: Lazy<ChatSetupController>): { agent: SetupAgent; disposable: IDisposable } {
		return instantiationService.invokeFunction(accessor => {
			...
			const setupAgentName = isMagnusDefaultChatAgent()
				? defaultChat.provider.default.name
				: `${defaultChat.provider.default.name} Copilot` /* Do NOT change for Copilot: hides the username altogether in Chat */;
			return SetupAgent.doRegisterAgent(instantiationService, chatAgentService, id, setupAgentName, true, description, location, mode, context, controller);
		});
	}
