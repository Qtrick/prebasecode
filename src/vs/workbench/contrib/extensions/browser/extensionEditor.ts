...
export class ExtensionEditor extends EditorPane {
	...
	private async renderMarkdown(extension: IExtension, cacheResult: CacheResult<string>, container: HTMLElement, token?: CancellationToken): Promise<string> {
		...
		// Honest empty state: blank gallery/local README must surface as "No README available."
...
class AdditionalDetailsWidget extends Disposable {
	...
	private renderExtensionResources(container: HTMLElement, extension: IExtension): void {
		...
		if (extension.url) {
			...
			const galleryLabel = typeof galleryUrl === 'string' && /open-vsx\.org/i.test(galleryUrl)
				? localize('Open VSX', "Open VSX")
				: localize('Marketplace', "Marketplace");
			resources.push([galleryLabel, ThemeIcon.fromId(Codicon.linkExternal.id), URI.parse(extension.url)]);
		}
	...
	private renderMarketplaceInfo(container: HTMLElement, extension: IExtension): void {
		...
		const galleryUrl = this.productService.extensionsGallery?.serviceUrl;
		const galleryInfoTitle = typeof galleryUrl === 'string' && /open-vsx\.org/i.test(galleryUrl)
			? localize('Open VSX Info', "Open VSX")
			: localize('Marketplace Info', "Marketplace");
		append(moreInfoContainer, $('.additional-details-title', undefined, galleryInfoTitle));
