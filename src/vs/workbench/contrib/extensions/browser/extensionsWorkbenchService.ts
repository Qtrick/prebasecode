	...
	async getReadme(token: CancellationToken): Promise<string> {
		...
		if (this.gallery) {
			if (this.gallery.assets.readme) {
				const content = await this.galleryService.getReadme(this.gallery, token);
				if (content && content.trim()) {
					return content;
				}
				// Asset was advertised but empty/unavailable — do not invent text.
...
**Notice:** This extension is bundled with PreBase. It can be disabled but not uninstalled.
## Features
${this.description}
`);
		}
