...
export class ExtensionStatusAction extends ExtensionAction {
	...
	private async computeAndUpdateStatus(): Promise<void> {
		...
		if (this.extension.state === ExtensionState.Uninstalled && this.extension.gallery && !this.extension.gallery.isSigned && shouldRequireRepositorySignatureFor(this.extension.private, await this.extensionGalleryManifestService.getExtensionGalleryManifest())) {
			const galleryUrl = this.productService.extensionsGallery?.serviceUrl;
			const unsignedMsg = typeof galleryUrl === 'string' && /open-vsx\.org/i.test(galleryUrl)
				? localize('not signed open vsx tooltip', "This extension is not signed. Open VSX packages are not Microsoft Marketplace-signed.")
				: localize('not signed tooltip', "This extension is not signed by the Extension Marketplace.");
			this.updateStatus({ icon: warningIcon, message: new MarkdownString(unsignedMsg) }, true);
			return;
		}
		...
		if (this.extension.missingFromGallery) {
			const galleryUrl = this.productService.extensionsGallery?.serviceUrl;
			const missingMsg = typeof galleryUrl === 'string' && /open-vsx\.org/i.test(galleryUrl)
				? localize('missing from open vsx tooltip', "This extension is no longer available on Open VSX.")
				: localize('missing from gallery tooltip', "This extension is no longer available on the Extension Marketplace.");
			this.updateStatus({ icon: warningIcon, message: new MarkdownString(missingMsg) }, true);
			return;
		}
