...
function getVersionAsset(version: IRawGalleryExtensionVersion, type: string): IGalleryExtensionAsset | null {
	...
	// Prefer the gallery-provided direct file URL when present (Open VSX
	// includes `source` for icons/README/etc.). Fall back to Marketplace-style
	// assetUri/{assetType} construction when source is absent.
	if (result.source) {
		return {
			uri: result.source,
			fallbackUri: result.source,
		};
	}
	return {
...
export abstract class AbstractExtensionGalleryService implements IExtensionGalleryService {
	...
	async getReadme(extension: IGalleryExtension, token: CancellationToken): Promise<string> {
		if (extension.assets.readme) {
			...
			// Honest empty: never invent README text when the gallery returns blank.
	...
	async getExtensionsControlManifest(): Promise<IExtensionsControlManifest> {
		...
		if (this.productService.defaultChatAgent?.extensionId &&
			this.productService.defaultChatAgent?.chatExtensionId &&
			!areSameExtensions({ id: this.productService.defaultChatAgent.extensionId }, { id: this.productService.defaultChatAgent.chatExtensionId })) {
