...
export class ExtensionGalleryManifestService extends Disposable implements IExtensionGalleryManifestService {
	...
	async getExtensionGalleryManifest(): Promise<IExtensionGalleryManifest | null> {
		...
		// Open VSX packages are not Microsoft Marketplace-signed. Claiming
		// allPublicRepositorySigned blocks real installs with NotSigned /
		// unverifiable Open VSX signatures. Keep true only for non-Open-VSX galleries.
		const isOpenVsx = /open-vsx\.org/i.test(extensionsGallery.serviceUrl);
		...
		return {
			...
			capabilities: {
				...
				signing: {
					allPublicRepositorySigned: !isOpenVsx,
				}
			}
		};
	}
