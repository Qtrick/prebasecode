	...
	private async downloadExtension(extension: IGalleryExtension, operation: InstallOperation, verifySignature: boolean, clientTargetPlatform?: TargetPlatform): Promise<{ readonly location: URI; readonly verificationStatus: ExtensionSignatureVerificationCode | undefined }> {
		if (verifySignature) {
			...
			if (isBoolean(value)) {
				...
				// Open VSX: Microsoft Marketplace signature checks do not apply.
				// Default off so real installs work; users can opt in via setting.
				const galleryUrl = this.productService.extensionsGallery?.serviceUrl;
				verifySignature = !(typeof galleryUrl === 'string' && /open-vsx\.org/i.test(galleryUrl));
			}
		}
