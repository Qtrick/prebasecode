...
// Local dump collection is opt-in via an explicit directory. PreBase never
// configures a crash endpoint or starts the reporter for uploads.
...
function configureCrashReporter(): void {
	...
	const companyName = (product.crashReporter ? product.crashReporter.companyName : undefined) || product.nameShort;
	// Keep local crash dumps available when explicitly requested, but never
	// configure a collection endpoint or upload them from a PreBase process.
