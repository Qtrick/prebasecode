...
const compilations = [
	...
	// prebase-magnus is compiled via dedicated `compile-magnus` / `watch-magnus`
	// so it is not listed here (avoids racing clean/compile of the same out/).
...
const createExtensionCompilationTasks = (tsconfigFile: string) => {
...
const tasks = compilations.map(createExtensionCompilationTasks);
// Dedicated Magnus tasks (used by `npm run compile-magnus` / `watch-magnus`),
// kept out of the aggregate compile-extensions parallel set to avoid races.
createExtensionCompilationTasks('extensions/prebase-magnus/tsconfig.json');
