...
const rootNpmrcConfigKeys = [
	...getNpmrcConfigKeys(path.join(root, '.npmrc')),
	...getNpmrcConfigKeys(path.join(root, '.npmrc-gyp')),
];
...
function setNpmrcConfig(dir: string, env: NodeJS.ProcessEnv) {
	for (const fileName of ['.npmrc', '.npmrc-gyp']) {
		const npmrcPath = path.join(root, dir, fileName);
...
function configureGitIfPresent(): void {
	// Skip git config when this tree isn't a git checkout (e.g. zip extract).
	if (!fs.existsSync(path.join(root, '.git'))) {
		log('.', 'Skipping git config (no .git directory)');
		return;
	}
