...
export function spawnTypeScriptCompiler(projectPath: string, config: { taskName: string; noEmit?: boolean }, onComplete?: () => Promise<void> | void): Promise<void> {
	...
	// TypeScript 7 CLI (`tsc` from `@typescript/native`).
...
export function createTypeScriptCompilerStream(projectPath: string, config: { taskName: string; noEmit?: boolean }, onComplete?: () => Promise<void> | void): NodeJS.ReadWriteStream {
	const stream = es.through();

	spawnTypeScriptCompiler(projectPath, config, onComplete).then(() => {
		stream.emit('end');
	}).catch(err => {
