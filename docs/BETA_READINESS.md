...
- **Magnus notch flicker & hover-only haptic constraint:** Fixed in `native/prebase-live-activity` — `refreshContentOnly` now calls `refreshContentSubviewsPreservingPresentation` without clearing `transitionInFlight` or incrementing `layoutCompletionGeneration`, preventing cancellation of in-flight window-frame animation completion handlers and skipped control layouts. Enforced strict hover-only haptics (zero haptics for clicks, submissions, options, approvals, denials, pin/unpin, Escape, attention arrival, content storms, and geometry transitions; exactly 1 haptic on 180ms hover acquisition dwell). Direct AppKit perf suite **PASS (22/22)** including `hover-only-haptic-policy` and `mid-morph-content-and-interactive-growth`. Live AppKit acceptance **PASS (CURRENT_GREEN)** in `reports/graph-acceptance/phase-3-final/magnus/live-activity.json` with native panel creation, valid PNG screenshot, and clean quit.
- **Emmet image-size security:** Hardened in `extensions/emmet/src/imageSizeHelper.ts` with strict raster format whitelist (`.png`, `.jpg`, `.jpeg`, `.gif`, `.webp`), SSRF loopback/private/link-local/cloud-metadata filtering, 5MB buffer cap, and 5000ms request timeout.
- **Licenses & CI hygiene:** Added `@typescript/typescript6` override in `cglicenses.json`; sanitized `pr-darwin-test.yml`, `pr-linux-test.yml`, and `pr-win32-test.yml` to prevent failures when `extensions/copilot` is absent.
...
Prior campaign notes (live AppKit acceptance, soak on older heads, Gemini 3.8 policy, launch inspector decoupling, graph mode switching) remain useful history but are **not** automatic proof for the current worktree.
...
| BETA-012 | Magnus | Ask/Plan/Edit/Agent + graph tools reliability | Critical | Needs Verification | — | GUI + Magnus ext | Tool smoke | 2026-09-04: **Notch flicker & hover-only haptics closed** — content updates separated from presentation morphs in native Live Activity without clearing in-flight animations or layout completion generation; zero haptics on clicks/actions/content storms and exactly 1 on hover dwell; 22/22 direct AppKit perf tests PASS; live AppKit acceptance PASS (CURRENT_GREEN) in `live-activity.json` with native panel, screenshot, and clean quit. Full provider-backed Agent/chat GUI matrix still open — **not Complete**. | Pre-beta | 2026-07-20 | — |
...
| BETA-023 | Security | Dependency audit + supply-chain review | Critical | In Progress | — | Electron/native packaged smoke | `npm audit` + review | 2026-09-04: Emmet `image-size` security hardened (safe rasters whitelist, SSRF private/local filtering, 5MB limit, 5000ms timeout); root audits clean; Electron **42.8.0**; packaged/native smoke still open — [SECURITY_AND_SUPPLY_CHAIN.md](SECURITY_AND_SUPPLY_CHAIN.md) | Pre-beta | 2026-07-20 | — |
...
| BETA-025 | Release | Third-party notices / license inventory for new packages | Medium | In Progress | — | Legal review | Notices update | `@typescript/typescript6` in lockfile and added to `cglicenses.json`; upstream CI pulls CG NOTICE before package — PreBase release pipeline **not wired**; see [PACKAGING.md](PACKAGING.md) | Pre-beta | 2026-07-20 | — |
