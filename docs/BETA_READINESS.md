...
| BETA-012 | Magnus | Ask/Plan/Edit/Agent + graph tools reliability | Critical | Needs Verification | — | GUI + Magnus ext | Tool smoke | 2026-09-04: **Notch flicker root cause closed** — content updates separated from presentation morphs in native Live Activity (`content-update-stability` + peek/interactive/pinned storms PASS; 22/22 direct AppKit perf). Morph stress ~201 anim/100 cycles (was ~501). Live Electron GUI re-verify still required outside Cursor agent (RegisterApplication SIGABRT under agent). Full provider-backed Agent/chat GUI matrix still open — **not Complete**. | Pre-beta | 2026-07-20 | — |
