...
- **Magnus notch product polish (in progress, not Complete):** Natural notch width + optical top-shoulder inset (non-degenerate corners without fixed-width slab); true-black fill + synchronized shape mask; truthful `contentViewport`=`contentScrollFrame`; geometrySignature pins interactive height against text churn; action IDs survive ledger→NAPI with keyed row reconcile; pending approval/question hides activity/log under controls; contentFooterGutter; max height 192. AppKit perf **42/42 PASS**; live AppKit acceptance **PASS** (real PreBase launch + native diagnostics). Remaining: dense question content viewport still tight; desktop `screencapture` often misses floating NSPanel pixels (Screen Recording / window-level); Sapphire external app not integrated; provider-backed Agent GUI matrix open — **BETA-012 not Complete**.
...
| BETA-012 | Magnus | Ask/Plan/Edit/Agent + graph tools reliability | Critical | Needs Verification | — | GUI + Magnus ext | Tool smoke | 2026-09-04: Notch polish iteration (natural width, measured 72–192 height, populated-first-paint, footer no-overlap, 2-col options, intentional peek copy, truthful Approve/Deny in-flight). AppKit perf + live acceptance **PASS** with screenshots under `reports/graph-acceptance/phase-3-final/screenshots/magnus-live-activity-*.png`. Provider-backed Agent/chat GUI matrix and remaining optical polish still open — **not Complete**. | Pre-beta | 2026-07-20 | — |
