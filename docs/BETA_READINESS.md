...
Last audit: **2026-09-03** (campaign: LA contracts in `assurance:quick` + macos CI; Darwin package/preLaunch fail-closed on missing AppKit `.node`; live harness requires follow-up/question/approval/blur+`prebaseForeground`; sticky Escape/unlock product truth; C3/Focus false greens closed; campaign CURRENT_GREEN/EXTERNAL on Linux; **full macOS AppKit live CURRENT_GREEN still required** — beta candidate claim still **forbidden**)
