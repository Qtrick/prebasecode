...
Last audit: **2026-09-04** (HEAD `cd7225df` + uncommitted Magnus notch polish iteration: natural width, measured height 72–164, populated-first-paint, footer/content no-overlap, 2-col options, intentional peek copy)

### CURRENT STATUS (authoritative)

- **Magnus notch product polish (in progress, not Complete):** Native AppKit surface uses natural notch width (pad only for question option rows), measured content height (min 72 / max 164 overflow guard + scroll), content staged at target size before morph (`contentPopulated`, no blank black expansion), path-aware insets, SF Symbol pin/open composer, shape-aware `hitTest`, truthful Approve/Deny in-flight (titles retained + alpha), 2-column option grid, and intentional short peek copy for long pending titles. Direct AppKit perf **PASS**; live AppKit acceptance **PASS** with populated visual fixtures under `reports/graph-acceptance/phase-3-final/screenshots/magnus-live-activity-*.png`. Remaining: further shoulder optical refinement; scroll chrome density on dense approval/question; full provider-backed Agent GUI matrix still open — **BETA-012 not Complete**.
- **Magnus notch geometry & native AppKit surface:** Continuous notched path (14-element topology); prefer zero-flare when width == compact span; softer S-curve when option pads widen; window frame & shape sync; layer-backed controls.
...
| BETA-012 | Magnus | Ask/Plan/Edit/Agent + graph tools reliability | Critical | Needs Verification | — | GUI + Magnus ext | Tool smoke | 2026-09-04: Notch polish iteration (natural width, measured 72–164 height, populated-first-paint, footer no-overlap, 2-col options, intentional peek copy, truthful Approve/Deny in-flight). AppKit perf + live acceptance **PASS** with screenshots under `reports/graph-acceptance/phase-3-final/screenshots/magnus-live-activity-*.png`. Provider-backed Agent/chat GUI matrix and remaining optical polish still open — **not Complete**. | Pre-beta | 2026-07-20 | — |
