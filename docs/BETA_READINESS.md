...
Last audit: **2026-08-31** (campaign continuation: Project Guidance OpenCode slash IDs + Gemini inert `!{}`/`@{}` security tests + Continue regex honesty; Live Activity Magnus-attributed metrics/questions/native options; soak false-green gate fixed; Live Activity liveGui now runtime diagnostics not source-contracts-only; Phase 3 overall gate still **not** green — beta candidate claim still **forbidden**)
...
| BETA-010 | Core | Clean startup / workspace open / crash recovery | Critical | Needs Verification | — | GUI launch | Fresh + existing profile launch | 2026-09-03: Live Core IDE acceptance (`prebase-core-ide-live.mjs`) PASS (C1–C5 clean launch, workspace folder mount, window reload driver re-attach, clean quit); full headless matrix validated. | Pre-beta | 2026-07-20 | — |
...
| BETA-011 | Core | Native editor reliability (save, undo, search, SCM, terminal, debug) | Critical | Needs Verification | — | GUI smoke | Manual smoke | 2026-09-03: Live Core IDE acceptance (`prebase-core-ide-live.mjs`) 35/35 criteria PASS (E1 save/undo editor disk sync, E2 workspace text query, E3 SCM status tracking, E4 interactive terminal execution and buffer inspection, E5 Node.js debug session launch, toolbar visibility and clean stop, E6 TS suggest/diagnostics, P1/P4/P5 views, N1–N15 Code Graph 4 layout modes, 3D interaction, camera-invariant node drag, pointer capture clean release). | Pre-beta | 2026-07-20 | — |
| BETA-012 | Magnus | Ask/Plan/Edit/Agent + graph tools reliability | Critical | Needs Verification | — | GUI + Magnus ext | Tool smoke | 2026-09-01 campaign VIII: Magnus unit suite **329** pass + 98 node acceptance; Native Notch motion architecture corrected: topology-compatible CAShapeLayer paths (single continuous contour with identical segment structure), explicit semantic target state (`targetExpanded`), delayed control layout to prevent animation popping, transition generation token to prevent stale completion races, explicit shape layer geometry synchronization, factual diagnostics (removed marketing booleans), improved perf harness (real animation cycles, proper hidden baseline, CPU/RSS measurement). Real macOS CPU/wakeup profiling with Instruments still pending. Full provider-backed Agent/chat GUI matrix still open — **not Complete**. | Pre-beta | 2026-07-20 | — |
...
| BETA-014 | Privacy | Telemetry/crash/surveys disabled; secrets storage; CSP | Blocker | In Progress | — | Runtime network observation (GUI) | Privacy checklist + `verify:privacy` | 2026-09-03: Live privacy runtime observation (`prebase-privacy-runtime.mjs`) PASS with 0 failures; verified process tree attribution with `lsof -a -P -iTCP -p <tree>`, CDP network observer interception with hostname identity resolution, 0 forbidden telemetry/crash/survey hosts, 0 unexpected destinations, clean exit without SIGKILL. | Pre-beta | 2026-07-20 | — |
