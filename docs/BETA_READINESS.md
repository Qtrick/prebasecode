...
Last audit: **2026-08-30** (graph visual recovery: Radial/Temporal readability + screen-aware render math; commit-tier domain precision so layout edits do not pull release soaks; live product-path/load-quit/soak **not** re-run this commit; beta candidate claim still **forbidden**)
...
| BETA-004 | Graphs | Active Code Graph manual acceptance (layouts, interaction, selection, idle rotation) | Critical | Needs Verification | — | GUI smoke | Manual smoke on the active Code Graph | 2026-08-30 visual recovery: live dogfood on real PreBase (~280 files / 420 edges) captured Organic→Radial Fit metrics (`reports/graph-acceptance/phase-3-final/visual-recovery/`); Radial redesigned (concentric rings + packed components, no global collision scale); screen-aware network radii unified with production webview. **Not Complete** — residual Radial extent vs Organic taste risk; 5-node acceptance alone is insufficient; full GRAPH_ACCEPTANCE matrix still open | Pre-beta | 2026-07-20 | — |
| BETA-005 | Graphs | Graph unit tests (`architecturePick`, `networkLayout`, ownership boundary) under `graphs/src/tests/unit` | High | Needs Verification | — | CI gate green on PR | Mocha/node harness green | 2026-08-30: **57 test files** pass locally via `npm run test:graphs` (incl. `graphVisualRecovery`, network Fit/radius parity, Temporal compact layout + aggregate lane contracts, layout-version invalidation). PR job green not confirmed — **not Complete**. | Pre-beta | 2026-07-20 | — |
...
| BETA-012 | Magnus | Ask/Plan/Edit/Agent + graph tools reliability | Critical | Needs Verification | — | GUI + Magnus ext | Tool smoke | 2026-08-30: Magnus unit suite **257** pass; live `magnus-guidance-smoke` PASS (~12s) — AGENTS/Cursor/Claude/Gemini `TEAM.md`/Kiro/OpenCode local+remote diagnostic, JIT graph path delta, skill+playbook activate, clean quit (`reports/graph-acceptance/phase-3-final/magnus/project-guidance-smoke.json`). Project guidance Settings/Quick Pick + `prebase_project_guidance` native tool. Full provider-backed Agent/chat GUI acceptance still open — **not Complete**. | Pre-beta | 2026-07-20 | — |
...
| BETA-024 | Graphs | Large-project graph performance/memory **and** visual readability at Fit View | High | In Progress | — | GUI FPS + memory + screen-space readability | Perf sample + visual metrics | Perf (2026-08-25 Phase 3.12): 280-node GUI FPS/memory evidence in `docs/GRAPH_PERFORMANCE.md`. Visual (2026-08-30): screen-aware node/label policy + Radial/Temporal macro recovery; live Fit metrics in `visual-recovery/`; **distinguish** from perf — readability still Needs Verification for large Temporal Full Map (k≈0.21 on ~9.7k nodes) and Radial vs Organic taste. | Pre-beta | 2026-07-20 | — |
