...
Last audit: **2026-09-04** (campaign: macOS AppKit Live Activity live acceptance **CURRENT_GREEN** on macOS with real native panel, screencapture, and live blur diagnostics; direct AppKit perf 11/11 pass; single-pane Graph Editor Temporal↔Network dynamic switching lifecycle hardened; build sandbox resilience unit test suite added; launch skill inspector decoupled from CDP; full 10-minute active soak **PASS** on current source; Hosted Magnus feature-gated `PREBASE_HOSTED_MAGNUS_ENABLED=false` with streaming TransformStream settlement; Gemini 3.8 Flash model policy aligned; signing/notarization remains external blocker)
...
| BETA-012 | Magnus | Ask/Plan/Edit/Agent + graph tools reliability | Critical | Needs Verification | — | GUI + Magnus ext | Tool smoke | 2026-09-04: Magnus unit suite **272** pass + 11/11 direct AppKit perf tests pass (100 morph cycles, 0 monitor leaks, zero stale expanded states, path topology invariant verified); macOS AppKit Live Activity live acceptance **CURRENT_GREEN** (`prebase-magnus-live-activity-live.mjs`: native panel created, visible, live screencapture verified, blur diagnostics verified); `gemini-3.8-flash` primary flagship model policy aligned; direct Gemini parameter compatibility for Gemini 3.x implemented. Full provider-backed Agent/chat GUI matrix still open — **not Complete**. | Pre-beta | 2026-07-20 | — |
