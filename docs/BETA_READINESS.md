...
| BETA-012 | Magnus | Ask/Plan/Edit/Agent + graph tools reliability | Critical | Needs Verification | — | GUI + Magnus ext | Tool smoke | 2026-08-31 campaign VII: Magnus unit suite **329** pass + 98 node acceptance; Native Notch motion truth fully established via AppKit `CAShapeLayer` continuous parameterized bezier paths with shoulder curves (`kShoulderRadius=12`, `kBottomCornerRadius=16`), presentation-layer retargeting (`CABasicAnimation`), layered subview staging, event monitoring 0% idle CPU (global monitor active only in collapsed mode with `lastInside` cache, disabled when expanded, restored on collapse), strict user-initiated haptic policy (`didHoverHaptic` single-fire guard), >4 question options handling with tertiary delegate button ("More in PreBase…"), and coarse elapsed boundary timer (no 1Hz loop). Static, live AppKit runtime, and perf soak (`prebase-live-activity-perf.mjs`) all pass (`failures: []`). Full provider-backed Agent/chat GUI matrix still open — **not Complete**. | Pre-beta | 2026-07-20 | — |
