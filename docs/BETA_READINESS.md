...
Last audit: **2026-08-23** (Phase 3.10 — graph production-truth, interaction integrity, a11y, settings honesty; beta candidate claim still **forbidden** pending GUI acceptance)
...
| BETA-005 | Graphs | Graph unit tests (`architecturePick`, `networkLayout`, ownership boundary) under `graphs/src/tests/unit` | High | Needs Verification | — | CI gate green on PR | Mocha/node harness green | 2026-08-23: **42 test files** discovered by `graphs/scripts/run-unit-tests.mjs` all pass locally (incl. production-webview runtime suites + network edge parity contract); PR job green not confirmed here. Phase 3.9 report's "44" was inaccurate prose; the archived tree had 41 files plus an untracked parity test = 42 today | Pre-beta | 2026-07-20 | — |
...
| BETA-018 | Quality | Accessibility (keyboard, HC, reduced motion) for graphs + IDE | High | In Progress | — | Assurance | A11y smoke | 2026-08-23 Phase 3.10: graph canvas focusable (`role=application` + roledescription/label/help), arrow-key node traversal with live-region announcements, Enter opens details, Escape/focus-return, F1 help panel, `aria-expanded` details toggle, SVG toolbar labels, reduced-motion respected in temporal transitions + camera anims + system preference listener. IDE-wide a11y and screen-reader GUI evidence remain open | Pre-beta | 2026-07-20 | — |
...
| BETA-024 | Graphs | Large-project graph performance/memory | High | In Progress | — | GUI FPS + memory | Perf sample | CPU layout micro-benchmark [GRAPH_PERFORMANCE.md](GRAPH_PERFORMANCE.md) 2026-08-13: radial ~11 ms @ 280 and ~144 ms @ 2,000 stress nodes; workbench caps layout work at 600. Phase 3.10 (2026-08-23) landed render-path structural work (paired z-sort, memoized temporal visibility, cached theme tokens, dead-cache removal) — in-product frame/memory evidence still not recorded | Pre-beta | 2026-07-20 | — |
