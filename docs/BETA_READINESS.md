...
| BETA-010 | Core | Clean startup / workspace open / crash recovery | Critical | Needs Verification | — | GUI launch | Fresh + existing profile launch | 2026-09-03: Core IDE harness strengthened for C1–C5 (workspace identity, crash recovery relaunch with same profile, reload). **Live producer on current HEAD not re-run in this Linux campaign** — prior PASS claims are stale until regenerated. | Pre-beta | 2026-07-20 | — |
...
| BETA-011 | Core | Native editor reliability (save, undo, search, SCM, terminal, debug) | Critical | Needs Verification | — | GUI smoke | Manual smoke | 2026-09-03: E6 false-green closed (`suggestSeen` alone can no longer pass; requires TS lang + controlled diagnostic via marker diagnostics). E5 requires active debug session count. P1 requires setting persistence. **Live evidence on current HEAD not regenerated here.** | Pre-beta | 2026-07-20 | — |
...
| BETA-027 | Quality | CI gates: graph boundary verifier + icon checksum diff | Critical | In Progress | — | Merge + green CI | CI fails on boundary/icon/privacy drift | 2026-09-03: Restored missing `.github/workflows/pr.yml` job `prebase-assurance` (`assurance:quick` + `assurance:privacy`). Workflow was absent from HEAD despite docs claiming it. Green CI run on this branch not yet confirmed. | Pre-beta | 2026-07-20 | — |
