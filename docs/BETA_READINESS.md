...
| BETA-012 | Magnus | Ask/Plan/Edit/Agent + graph tools reliability | Critical | Needs Verification | — | GUI + Magnus ext | Tool smoke | 2026-08-31 campaign: Magnus unit suite **271** pass; Project Guidance V2 corrected (OpenCode slash namespaces, Gemini `!{}`/`@{}` inert canaries, Continue `regex` Partial/unsupported); Live Activity data plumbing + native question options; live producer now `liveGui=workbench-diagnostics-runtime` (native-appkit backend + session status) — not source-contracts-only. Full provider-backed Agent/chat GUI + native panel screenshot matrix still open — **not Complete**. | Pre-beta | 2026-07-20 | — |
