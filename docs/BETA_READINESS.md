...
- **Magnus notch product polish (in progress, not Complete):** Natural notch width + C1 continuous optical top-shoulder fillets (smooth tangency at top edge and vertical sides; 14-element invariant path topology); true-black fill (`calibratedWhite: 0.0 alpha: 1.0` in all states, eliminating gray attention clash); unified 22pt horizontal alignment grid across headers, messages, action rows, option buttons, and composer; equal-width 2-column question option grid; eliminated duplicate footer gutters expanding scroll viewport to >=33pt (eliminating false-green test skips); 10.5pt semibold compact wing typography without truncation; window ID (`screencapture -l`) screenshot capture with IHDR dimension validation (rejecting full-desktop fallbacks); truthful `contentRefreshCount`. AppKit perf **42/42 PASS** (0 skipped tests); unit Mocha **126/126 PASS**; contracts **8/8 PASS**; live AppKit acceptance **PASS** with verified 626x68 panel window captures under `reports/graph-acceptance/phase-3-final/screenshots/magnus-live-activity-*.png`. Remaining: external Sapphire app integration optional reference; provider-backed Agent GUI matrix open — **BETA-012 not Complete**.
