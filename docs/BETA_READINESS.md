...
Last audit: **2026-09-03** (campaign: cold-start lock query + native lock hide + macos AppKit CI job; sticky Escape/unlock product truth; C3/Focus false greens closed; campaign CURRENT_GREEN/EXTERNAL on Linux; **full macOS AppKit live CURRENT_GREEN still required** — beta candidate claim still **forbidden**)
...
| BETA-027 | Quality | CI gates: graph boundary verifier + icon checksum diff | Critical | In Progress | — | Merge + green CI | CI fails on boundary/icon/privacy drift | 2026-09-03: `prebase-assurance` on ubuntu + new `magnus-live-activity-macos` (compile AppKit addon + static/perf). Green CI run on this branch not yet confirmed; full AppKit live producer still open. | Pre-beta | 2026-07-20 | — |
