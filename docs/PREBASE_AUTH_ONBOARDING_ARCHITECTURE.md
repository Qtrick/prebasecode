...
The account service owns a single OAuth PKCE transaction: it generates a local verifier plus a non-secret application flow ID, puts `sb_flow_id` only in the allowlisted `redirect_to`, opens the system browser, consumes only the exact `prebase://auth/callback` authority/path with that flow ID, exchanges the one-time code, and writes only the resulting PreBase access/refresh session through the existing SecretStorage adapter. Supabase owns the provider OAuth `state`; PreBase does not override it. Transactions expire after five minutes and are never persisted or logged. GitHub and Google are the only public providers.
