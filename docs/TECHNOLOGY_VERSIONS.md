...
| `@typescript/native-preview` / `tsgo` CLI | **Removed** from root lockfile | Superseded by `typescript@7.0.2` | Same as above | Replaced by stable `@typescript/native` alias | Do not reintroduce | Removed during BETA-006 install | Gulp/helpers still named `tsgo.ts` but spawn `tsc` |
...
| npm | 11.13.0 (local); notice of 12.0.1 available | 12.0.1 | [npm releases](https://github.com/npm/cli/releases) | OK | **Defer** | Deferred | Not required for TS7 |
...
| Bundler | gulp / build/next / rspack / vite (build tools) | — | — | Primary `tsc` in gulp stream | Repair as needed | `build/lib/tsgo.ts` → `npx tsc` | Full `compile-extensions` not in assurance |
...
- `build/lib/tsgo.ts` / gulp extension pipelines spawn **`tsc`** (TS7), not `tsgo` / native-preview.
...
- Application icons are protected; baseline checksums: `/tmp/prebase-icon-protection/checksums.sha256` (100 paths, verified 2026-07-20).
